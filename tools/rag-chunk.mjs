...
const 제목규칙 = [
  ...
  { re: /^(\[[^\]]{1,30}\])\s*$/, 깊이: () => 1 },                   // [부록 A]
  // ASME 코드북 (2026-09-19 추가). 이걸 안 넣으면 700쪽이 제목 없이 뭉텅이로 잘려
  // 조각만 봐서는 어느 조항인지 알 수 없다 — AI 가 근거를 못 댄다.
  { re: /^(PART\s+[A-Z]{1,3}\b.*)$/, 깊이: () => 1 },                // PART QG GENERAL REQUIREMENTS
  { re: /^(ARTICLE\s+[IVXLC]+\b.*)$/, 깊이: () => 1 },               // ARTICLE I WELDING GENERAL REQUIREMENTS
  { re: /^([A-Z]{1,3}-\d+(?:\.\d+)*(?:\s+[A-Z][A-Za-z0-9().,\/'’\- ]*)?)$/, 깊이: () => 2 },  // QW-140 · UG-27 · QW-403.1
  // 5. / 5.2 / 5.2.1 — **제목 글이 글자로 시작할 때만**.
  // 안 그러면 ASME 표의 "75 (515)" · "8.1  130" 같은 줄을 전부 제목으로 읽어
  // 조각이 한 글자짜리로 부서진다(2026-09-19 Sec.IX 2,930조각·평균 271자로 발각).
  // 번호:true = 표가 많은 문서에서는 이 규칙만 끌 수 있다(조각내기 4번째 인자 {번호제목:false}).
  // 그래도 "18 UNF" · "51 through P-No. 53" 같은 표 줄은 남는다 — ASME 는 QW-/UG- 규칙이
  // 따로 있으므로 꺼도 잃는 게 없다. 한국어 규정은 이게 주된 제목이라 기본은 켬이다.
  // 최대 40자: 한국 규정은 **본문 문단도 번호로 시작한다**("3.2.7.1 고온의 운전조건에서 사용하는 …").
  // 80자를 그대로 두면 그런 문장이 제목이 되어, 깊이 1 짜리가 걸리는 순간 그 아래 조각들이 전부
  // "3. 결함과 결함과의 간격이…" 같은 엉뚱한 경로를 달게 된다(KGS AC111 실측). 진짜 제목은 짧다.
  { 번호: true, 최대: 40, re: /^(\d+(?:\.\d+){0,3})\.?\s+([([]?[A-Za-z가-힣][^\n]*)$/, 깊이: (m) => 1 + (m[1].match(/\./g) || []).length },
];
export function 제목인가(줄, { 번호제목 = true } = {}) {
  const t = String(줄 || '').trim();
  if (!t || t.length > 80) return null;                 // 긴 줄은 본문이다
  if (/[.?!]$/.test(t) && !/^\d/.test(t)) return null;  // 마침표로 끝나면 문장
  for (const r of 제목규칙) {
    if (r.번호 && !번호제목) continue;
    if (r.최대 && t.length > r.최대) continue;
...
export function 조각내기(원문, docName, 최대 = 기본최대, 옵션 = {}) {
