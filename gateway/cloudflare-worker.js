...
// Firestore 를 못 읽을 때(한도·장애) 마지막으로 알던 범위를 얼마나 믿을지.
...
async function 볼수있는범위(env, auth) {
  ...
  try {
    ...
    // 재무부 자료가 통째로 안 보였고, 화면에는 아무 설명도 없어서 "왜 없다고 해?" 가 됐다.
    //
    // 한도가 차는 건 잠깐이고 권한이 바뀐 건 아니다. 그런데 좁은 쪽으로 떨어뜨리면
    // **되레 사람이 자료를 못 본다** — 안전해지는 게 아니라 고장 나는 것이다.
    // 그래서 최근에 확인해 둔 범위가 있으면 그걸 쓴다(최대 ${(마지막기억밀리초/3600000)}시간).
    // 그 시간이 지나면 그때는 진짜 모르는 것이니 좁은 쪽으로 간다.
    //
...
async function 맥검색(env, query, topK, 범위, 더 = {}) {
  ...
  return { 범위복구: j.범위복구, 되찾은범위: j.되찾은범위, 것: j.결과.map((c) => ({
    ...
    // 볼트(NAS 파일 카드)에서 온 것인지 표시한다 — 받는 쪽이 문턱을 달리 쓴다.
    // 코드북 조각과 파일 경로는 **점수 눈금이 다르다**: 상관없는 ASME 도 0.55 언저리까지 올라오는데
    // 볼트 경로는 맞는 답이어도 0.59~0.64 다. 한 문턱으로 자르면 둘 중 하나가 반드시 틀린다.
    kind: c.출처 === '볼트' ? '볼트' : '', recId: '',
    ...(c.쪽 ? { page: c.쪽 } : {}),
...
async function handleRag(request, env, path, cors) {
  ...
  if (path === 'table') {
    ...
    try {
      ...
      //   범위 칸도 같다: 맥은 늘 범위펴기(되살린 범위)를 싣는다 → 모르는 날엔 되찾은 배열의 넓힌 판이 샌다.
      //   모르는 날엔 /rag/search 처럼 게이트웨이가 아는 범위(전사)로 덮는다. 평소는 그대로(리뷰에서 발견).
