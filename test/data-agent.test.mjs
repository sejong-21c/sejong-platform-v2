...
// 9/26: 날짜말은 코드가 푼다 — 토요일(9/26)에 "다음 주 화요일" 을 모델이 9/30(수)로 셌다. 주는 월요일에 시작.
{
  ...
  const 토 = new Date(2026, 8, 26, 18, 30);               // 2026-09-26 토요일 저녁
  const 풀 = (q, d = 토) => 날짜말풀기(q, d);
  await T('다음 주 화요일 = 9/29(화)', () => assert.deepStrictEqual(풀('다음 주 화요일 10시에 품질회의 일정 잡아 줘'), ['다음 주 화요일 = 2026-09-29(화)']));
  await T('붙여 써도(담주·차주·다음주)', () => assert.deepStrictEqual(풀('담주 금요일').concat(풀('차주 월요일'), 풀('다음주 일요일')), ['담주 금요일 = 2026-10-02(금)', '차주 월요일 = 2026-09-28(월)', '다음주 일요일 = 2026-10-04(일)']));
  await T('이번 주는 이번 월~일(토요일에 이번 주 화요일 = 지난 9/22)', () => assert.deepStrictEqual(풀('이번 주 화요일 회의록'), ['이번 주 화요일 = 2026-09-22(화)']));
  await T('다다음 주·지난주', () => assert.deepStrictEqual(풀('다다음 주 수요일').concat(풀('지난주 목요일')), ['다다음 주 수요일 = 2026-10-07(수)', '지난주 목요일 = 2026-09-17(목)']));
  await T('요일만 말하면 다가오는 그날(오늘과 같은 요일이면 다음 주)', () => assert.deepStrictEqual(풀('화요일에 회의').concat(풀('토요일에', 토)), ['화요일(다가오는) = 2026-09-29(화)', '토요일(다가오는) = 2026-10-03(토)']));
  await T('내일·모레·N일 뒤·N주 후', () => assert.deepStrictEqual(풀('내일 오전').concat(풀('모레'), 풀('3일 뒤'), 풀('2주 후 점검')), ['내일 = 2026-09-27(일)', '모레 = 2026-09-28(월)', '3일 뒤 = 2026-09-29(화)', '2주 후 = 2026-10-10(토)']));
  await T('달을 넘겨도(10/31 토 → 다음 주 월 11/2)', () => assert.deepStrictEqual(풀('다음 주 월요일', new Date(2026, 9, 31)), ['다음 주 월요일 = 2026-11-02(월)']));
  await T('날짜말이 없으면 빈 목록', () => assert.deepStrictEqual(풀('교정 만료된 측정기구 몇 개야?'), []));
}

console.log(`data-agent 테스트 ${n}개 전체 통과 (의도 가르기 · 기간 · 묶어 세기 · 읽기 길 · 실패 안내 · 날짜말)`);
