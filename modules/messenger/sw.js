/* SJ 메신저 서비스워커 — W1: 설치 · 오프라인 재열림
 *
 * 규칙 하나: 여기서 건드리는 건 "모양이 변하지 않는 파일"뿐이다.
 *   - 같은 출처(sejong21c.com)의 정적 파일, 구글 폰트, gstatic 의 Firebase SDK → 캐시
 *   - Firestore/Auth(googleapis.com) → 손대지 않는다. 롱폴링 응답을 캐시하면 메시지가 멈춘다.
 *
 * 버전 문자열을 올리면 옛 캐시는 activate 때 전부 지운다. (messenger.html 을 고치면 올릴 것)
 */
const 버전 = 'sj-msg-v1';
const 껍데기 = [
  './messenger.html',
...
// 설치 시점에 미리 받아둬야 비행기 모드에서 확실히 열린다. 버전은 messenger.html 의 import 와 같아야 하고,
...
self.addEventListener('install', (e) => {
  e.waitUntil((async () => {
    ...
    await c.addAll(껍데기);                                   // 이건 실패하면 설치도 실패해야 한다
...
self.addEventListener('fetch', (e) => {
  ...
  // 나머지 정적 파일: 캐시 먼저 주고 뒤에서 갱신
  e.respondWith(
    caches.match(요청, { ignoreSearch: false }).then((캐시됨) => {
      const 네트 = fetch(요청)
        .then((r) => { 넣기(요청, r.clone()); return r; })
        .catch(() => 캐시됨);
      return 캐시됨 || 네트;
