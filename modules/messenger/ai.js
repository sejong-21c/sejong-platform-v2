...
// 같은 날 검토: 관문은 기록에 일곱 자리(topK-3)까지 주고 파이스는 세 자리만 받는 날이 있다 — 볼트와 달리 기록엔 몫 상한이 없다.
//   기록 말이 섞인 규격 물음('ISO 9001 시정조치 요구사항', 'KGS 부적합 판정 기준')에 기록을 다 지키니 네 자리에서 규격이 0 이 됐다.
//   그래서 기록 물음이면 규격 1위(기록도 볼트도 아닌 첫 조각)를 바닥(i - 2n)에 둬 기록보다도 나중에 뺀다(관문의 '맥은 최소 세 자리' 와 같은 뜻).
export const 문서순 = (문서, 물음) => {
  const 파일 = 파일물음인가(물음), 기록 = 기록물음인가(물음), 목록 = 문서 || [], n = 목록.length;
  const 규격1위 = 기록 ? 목록.findIndex((m) => !기록조각인가(m) && !(m && m.kind === '볼트')) : -1;
  return 목록.map((m, i) => (i === 규격1위 ? i - 2 * n : (파일 && m && m.kind === '볼트') || (기록 && 기록조각인가(m)) ? i - n : i));
};
