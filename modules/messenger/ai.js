 ...
 */
function 지침(맥락, perm, 표있다 = false, 화면들 = [], 고칠프로젝트 = []) {
  return [
    ...
    // ── 고치기 제안 (2026-09-23) ────────────────────────────────────────────
    // 목록은 **손으로 박지 않는다.** 부모 앱의 window.고칠수있는프로젝트() 가 권한까지 걸러 준다.
    //   9/22 에 화면 목록을 손으로 박아 뒀다가 실제와 어긋난 적이 있다 — 같은 실수를 반복하지 않는다.
    // 프로젝트가 없으면 이 절이 통째로 빠진다. 못 하는 일을 할 수 있다고 말하게 두면 안 된다.
    ...(고칠프로젝트.length
      ? ['',
        '## 스케줄 진척률 고치기',
        '이 사람이 **고칠 수 있는** 프로젝트: ' + 고칠프로젝트.map((p) => `${p.code || ''} ${p.name || ''}`.trim()).join(' · '),
        '"~ 완료 처리해줘" · "~ 진척 70%로 해줘" 처럼 **스케줄을 바꿔 달라는 부탁**이면,',
        '답 맨 끝에 아래 꼴의 덩이를 딱 하나 붙인다. 앞의 말은 한 줄로 짧게.',
        '```실행',
        '{"도구":"스케줄","프로젝트":"<위 목록의 이름이나 코드 그대로>","단락":"<단락 코드나 이름>","값":<0~100 정수>}',
        '```',
        '· "완료" = 100 · "착수"·"시작" = 0 이 아니라 사람이 말한 수를 쓴다. 수를 안 말했으면 묻는다.',
        '· **단락은 사람이 말한 그대로** 넘긴다("3", "3번 Tank 제작", "Tank 제작" 모두 좋다).',
        '  어느 항목인지 찾는 것은 플랫폼이 한다 — 네가 짐작해서 코드를 지어내지 마라.',
        '· 위 목록에 없는 프로젝트는 이 덩이를 만들지 마라. 고칠 권한이 없다고 답한다.',
        '· **몇 건이 바뀌는지, 바뀐다는 말은 쓰지 마라.** 네가 세는 것이 아니다.',
        '  덩이를 붙이면 확인 카드가 자동으로 붙고, 거기에 바뀔 항목이 전부 나온다.',
        '· 스케줄을 바꿔 달라는 말이 아니면 이 덩이를 절대 붙이지 마라(그냥 물어본 것이면 답만 한다).']
      : []),
    '',
    `## 지금 말하는 사람`,
...
// 여기서 하는 검사는 **꼴 검사뿐**이다 — 프로젝트가 실재하는지, 권한이 있는지, 무엇이 바뀌는지는
//   전부 부모 앱(window.스케줄제안)이 실제 자료를 보고 정한다. 모델 말을 믿고 쓰지 않는다.
export function 실행뽑기(글) {
  const s = String(글 || '');
  // ```실행 … ``` 또는 ```json … ``` 어느 쪽으로 싸도 받는다(모델이 종종 json 으로 쓴다).
  const m = s.match(/```(?:실행|json)?\s*(\{[\s\S]*?\})\s*```/);
  if (!m) return { 글: s, 제안: null };
  let o = null;
  try { o = JSON.parse(m[1]); } catch (e) { return { 글: s.replace(m[0], '').trim(), 제안: null }; }
  const 남은 = s.replace(m[0], '').trim();
  if (!o || o.도구 !== '스케줄') return { 글: 남은, 제안: null };
  // **Number() 에 그냥 넘기면 안 된다.** Number(null)·Number(false) 는 0 이고, 0 은 유효한 값
  //   (미시작)이라 그대로 통과한다 — 모델이 값을 못 채워 null 을 뱉으면 **진척률을 0 으로 지우는**
  //   카드가 뜬다. 시험이 이걸 잡았다(2026-09-23). 숫자이거나 숫자꼴 문자열일 때만 받는다.
  const 날값 = o.값;
  const 숫자다 = typeof 날값 === 'number' || (typeof 날값 === 'string' && /^\d{1,3}$/.test(날값.trim()));
  if (!숫자다) return { 글: 남은, 제안: null };
  const 값 = Math.round(Number(날값));
  // 범위 밖은 **자르지 않고 거절한다.** 150 을 100 으로 조용히 바꾸면, 사람은 카드에서 100 을 보고
  //   "내가 말한 대로" 라 여기고 누른다. 그건 확인이 아니다.
  if (!Number.isFinite(값) || 값 < 0 || 값 > 100) return { 글: 남은, 제안: null };
  const 프로젝트 = String(o.프로젝트 || '').trim();
  const 단락 = String(o.단락 || '').trim();
  if (!프로젝트 || !단락) return { 글: 남은, 제안: null };
  return { 글: 남은, 제안: { 도구: '스케줄', 프로젝트, 단락, 값 } };
}
...
export async function 답하기({ 질문, 히스토리 = [], 맥락 = '', 권한, fb, 표있다 = false, 화면들 = [], 고칠프로젝트 = [] }) {
  const sys = 지침(맥락, 권한, 표있다, 화면들, 고칠프로젝트);
