...
function 지침(맥락, perm) {
  return [
    ...
    // 이 줄이 없으면 위 규칙이 아래 맥락의 "화면 경로를 안내하지 마라" 를 이긴다.
    // 2026-09-22 실측: NAS 표에서 센 수를 맞게 답해 놓고 밑에 "왼쪽 메뉴 견적 →
    // NAS 견적서 목록" 이라는 **없는 화면**을 붙였다. 뒤에 적은 지시가 이기게 못 박는다.
    '**단, 아래 자료에 "화면 경로를 안내하지 마라" 고 적혀 있으면 그것이 이 규칙보다 우선이다.**',
