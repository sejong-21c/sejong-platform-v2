...
export const 셀것 = [
  ...
  // 2026-09-26 직원 시범 전 대조: 별칭 '반출' 이 "반출금지 도면 몇 장" 을 측정기구 반출입으로 끌고 갔다 — 좁힌다.
  { 이름: '측정기구 반출입', 컬렉션: 'measurementCheckouts', 별칭: ['반출입', '측정기구 반출'] },
...
// 2026-09-26 직원 시범 전 대조: "내 업무 몇 건이야?"·"이번 달 내 일정 몇 개야?" 가 세는 말 때문에 NAS 표로 갔다(0줄 → "찾지 못했다").
//   내 업무·일정은 맥락(messenger.js 내업무일정)이 이미 싣는다 — 찾기로 보낸다. 앞 글자가 한글이면 안 건다("사내 일정").
//   뒤돌아보기(?<!) 는 옛 아이폰 사파리가 못 읽어 파일째 죽으므로 쓰지 않는다.
const 내업무말 = /(^|[^가-힣])(내|나의|제)\s*(업무|일정|할\s*일)/;
export function 의도가르기(질문) {
  if (내업무말.test(String(질문 || ''))) return { 갈래: '찾기', 시킴: 시키는질문인가(질문) };
  const g = 다듬(질문);
...
//   물음에 업무·일정 말이 있으면 짚은 것으로 올린다. 표 갈래는 뺀다 — "회의비 합계" 의 NAS 표 줄(80)을 밀어내면 안 된다.
export const 업무급 = (물음, 갈래) => (갈래 !== '표' && /업무|일정|할\s*일|마감|회의/.test(String(물음 || '')) ? 급.짚은것 : 급.업무);
