...
function 지침(맥락, perm, 표있다 = false) {
  return [
    ...
    '**「A」 → 「B」 처럼 2단계 경로를 쓰지 마라.** 네가 아는 것은 위 목록의 이름뿐이고',
    '그 아래 하위 화면 이름은 모른다. 필요하면 1단계 이름 하나만 말한다.',
    ...(표있다
...
export async function 답하기({ 질문, 히스토리 = [], 맥락 = '', 권한, fb, 표있다 = false }) {
  const sys = 지침(맥락, 권한, 표있다);
